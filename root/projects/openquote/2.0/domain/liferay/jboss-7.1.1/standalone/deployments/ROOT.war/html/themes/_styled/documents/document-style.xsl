...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" version="2.0" xmlns:java="java">
    <xsl:import href="http://localhost:8080/html/themes/_styled/documents/generic-layouts-fo.xsl"/>
    <!--<xsl:import href="generic-insurance-objects-fo.xsl"/>-->
    <!-- imported styles-->
    <xsl:param name="style-doc" select="'styles.xml'"/>
    <xsl:variable name="styles" select="document('http://localhost:8080/html/themes/_styled/documents/styles.xml')"/>
    <!-- imported styles-->
    <xsl:template match="/">
        <fo:root xsl:use-attribute-sets="normal-font">
            ...
            <fo:page-sequence format="1" initial-page-number="1" master-reference="all-pages">
                <!-- before -->
                <fo:static-content display-align="before" flow-name="first-before">
                    <fo:block xsl:use-attribute-sets="header">
                        <xsl:value-of select="documentData/headerData/itemData"/>
                    </fo:block>
                </fo:static-content>
                <fo:static-content display-align="before" flow-name="blank-before">
                    <fo:block xsl:use-attribute-sets="header">
                        <xsl:text>This page has been left intentionally blank.</xsl:text>
                    </fo:block>
                </fo:static-content>
                <fo:static-content display-align="before" flow-name="right-before">
                    <fo:block xsl:use-attribute-sets="header" text-align="right">
                        <xsl:value-of select="documentData/headerData/itemData"/>
                    </fo:block>
                </fo:static-content>
                <fo:static-content display-align="before" flow-name="left-before">
                    <fo:block xsl:use-attribute-sets="header" text-align="left">
                        <xsl:value-of select="documentData/headerData/itemData"/>
                    </fo:block>
                </fo:static-content>
                <!-- after -->
                <fo:static-content display-align="after" flow-name="first-after">
                    <xsl:call-template name="footer">
                        <xsl:with-param name="rl" select="'centre'"/>
                    </xsl:call-template>
                </fo:static-content>
                <fo:static-content display-align="after" flow-name="right-after">
                    <xsl:call-template name="footer">
                        <xsl:with-param name="rl" select="'right'"/>
                    </xsl:call-template>
                </fo:static-content>
                <fo:static-content display-align="after" flow-name="left-after">
                    <xsl:call-template name="footer">
                ...
                <fo:table-row>
                    <fo:table-cell>
                        <xsl:choose>
                            ...
                            <fo:block text-align="left" xsl:use-attribute-sets="footer">Page&#x00A0;<fo:page-number/>&#x00A0;of&#x00A0;<fo:page-number-citation
                                    ref-id="TVLP"/></fo:block>
                            </xsl:when>
                    ...
                    <fo:table-cell>
                        <xsl:choose>
                            ...
                            <fo:block text-align="center" xsl:use-attribute-sets="footer">Page&#x00A0;<fo:page-number/>&#x00A0;of&#x00A0;<fo:page-number-citation
                                    ref-id="TVLP"/></fo:block>
                            </xsl:when>
                            <xsl:otherwise><fo:block>&#x00A0;</fo:block></xsl:otherwise>
                        </xsl:choose>
                    ...
                    <fo:table-cell>
                        <xsl:choose>
                            ...
                            <fo:block text-align="right" xsl:use-attribute-sets="footer">Page&#x00A0;<fo:page-number/>&#x00A0;of&#x00A0;<fo:page-number-citation
                                    ref-id="TVLP"/></fo:block>
                                    </xsl:when>
                            <xsl:otherwise><fo:block>&#x00A0;</fo:block></xsl:otherwise>
                            </xsl:choose>
    ...
    <xsl:template match="block">
        <xsl:variable name="column1">
            <xsl:choose>
                <xsl:when test="ancestor::block">4</xsl:when>
                    ...
                    <fo:table-header>
                        <fo:table-row>
                            <xsl:for-each select="itemData">
                                <fo:table-cell padding-start.length="1mm" xsl:use-attribute-sets="after-border end-border">
            ...
            <xsl:for-each select="block">
                <xsl:if test="@placement='footer'">
                    <fo:table-footer>
                        <fo:table-row>
                            <xsl:for-each select="itemData">
                                <fo:table-cell padding-start.length="1mm" xsl:use-attribute-sets="end-border">
                                    <fo:block>
                                        <xsl:value-of select="."/>&#x00A0;
                                    </fo:block>
                                </fo:table-cell>
