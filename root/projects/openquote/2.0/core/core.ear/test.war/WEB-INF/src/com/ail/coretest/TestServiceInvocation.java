...
import com.ail.core.configure.server.GetNamespacesArgImp;
import com.ail.core.configure.server.ServerBean;
import com.ail.core.configure.server.ServerEndpoint;
...
public class TestServiceInvocation extends CoreUserTestCase {
     ...
     */
    public TestServiceInvocation(String name) {
        super(name);
    }

    public static Test suite() {
        return new TestSuite(TestServiceInvocation.class);
    }
    ...
    public void testDroolsDecisionTable() throws Exception {
        ...
        assertTrue("caching isn't working - initial call should be at least 10s of times slower than later calls. (initial: "+(end1-start1)+", subsequent:"+(end2-start2)+")", ((double)(end2-start2))/(end1-start1) < 0.15);
    }
    ...
    /**
     * Invoke the configure server via its web service interface. This test uses
     * the web service directly (i.e. not via any accessor).
     * THIS TEST SKIPPED UNTIL WE NEED WEB SERVICE SUPPORT
     * @throws Exception
     */
    public void DONTtestSimpleWebServiceClient() throws Exception {
        URL url=new URL("http://localhost:8080/configure-server-ejb/Configure?wsdl");
        
        QName qname = new QName("http://com.ail.core.configure.server", "ConfigureService");

        ServiceFactory factory=ServiceFactory.newInstance();
        Service service=factory.createService(url, qname);
        
        ServerEndpoint endpoint=(ServerEndpoint)service.getPort(ServerEndpoint.class);
        
        GetNamespacesArg arg = new GetNamespacesArgImp();

        String argAsString = getCore().toXML(arg).toString();
        String ret = endpoint.invokeServiceXML(argAsString);
     ...
     * @throws Exception
     */
    public void DONTtestWebServiceWithUrlViaAccessor() throws Exception {
        GetConfigurationCommand command = (GetConfigurationCommand) getCore().newCommand("TestWebService1");
        command.setNamespaceArg("com.ail.core.Core");
        command.invoke();
        assertNotNull(command.getConfigurationRet());
    }
     ...
     * applys a simple rule: <code>if x&lt;100 r=x+y; else r=2*x+y;)</code>. The inheriting service 
