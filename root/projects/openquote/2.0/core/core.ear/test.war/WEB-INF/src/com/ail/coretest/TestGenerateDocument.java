    ...
    public void testDocGenService() throws Exception {
        ...
        try {
            ...
            for(int i=0 ; i<10 ; i++) {
                ...
                out=new PrintWriter(new File("../../TestGenerateDocument"+i+".pdf"));
                out.print(doc);
                out.close();
            }
        }
...
//        out.close();
        ...
        assertNotNull(doc);
        // Check a range here because the actual length will vary a bit with the date.
        assertTrue("Generated document's size ("+doc.length+" bytes) is unexpected.", doc.length > 4720 && doc.length < 4760 );
    }
}
