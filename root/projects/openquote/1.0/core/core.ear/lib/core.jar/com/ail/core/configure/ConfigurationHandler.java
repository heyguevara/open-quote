...
public class ConfigurationHandler {
     ...
     */
	private Configuration findConfiguration(String owningNamespace, CoreUser user, Core core) {
    ...
    public Collection<String> getConfigurationSources(ConfigurationOwner owner, CoreUser user, Core core) {
        ...
        if (!Core.CoreNamespace.equals(config.getNamespace())) {
            config=findConfiguration(Core.CoreNamespace, user, core);
            sources.add(config.getSource());
        }
    ...
    public Collection<String> getConfigurationNamespaceParent(ConfigurationOwner owner, CoreUser user, Core core) {
        ...
        if (!Core.CoreNamespace.equals(config.getNamespace())) {
            sources.add(Core.CoreNamespace);
        }
     ...
     * @param owner The class owning the configuration
     * @param user The class using the configuration
     * @param core The core used by the user
     * @return The XMLMapping, or null if none is defined.
     */
    public XMLMapping getXMLMapping(ConfigurationOwner owner, CoreUser user, Core core) {
        XMLMapping ret=null;

        try {
	        Configuration config=findConfiguration(owner.getConfigurationNamespace(), user, core);
	        ret=config.getXmlMapping();
        }
        catch(UnknownNamespaceError e) {
	        if (owner==core) {
                throw e;
        ...
        // If the group wasn't in the config user's namespace, is it in the core
        // namespace?
        if (ret==null && owner!=core) {
	        ret=getXMLMapping(core, user, core);
        }
