...
/**
 * <p>
 * An AttributeField represents an individual column within a
 * {@link RowScroller}. The RowScroller itself iterates over a collection of
 * results rendering one row on the screen for each row of data in the
 * collection. The columns in each row are represented by AttributeFields which
 * are bound to {@link com.ail.core.Attribute Attributes} in the quotation.
 * </p>
 * <p>
 * <img src="doc-files/AttributeField.png"/>
 * </p>
 * <p>
 * In the example above the RowScroller is bound to a collection of vehicle
 * assets. The current quote contains two assets matching the criteria - an Alfa
 * Romeo and a Bentley. The RowScroller contains nine AttributeFields each of
 * which is bound to a different Attribute in the assets. The AttributeField
 * bindings are taken to be relative to the binding of the RowScroller. In this
 * case the RowScroller is bound to <code>asset[assetTypeId='Vehicle']</code>,
 * and the individual AttributeFields are bound to
 * <code>attribute[id='Make']</code>, <code>attribute[id='Model']</code>, etc
 * </p>
 * <p>
...
public class AttributeField extends PageElement {
    ...
    /**
     * Render an AttributeField on the UI. This is quite a common requirement
     * throughout the classes of the ui package, so it's put here for
     * convenience. The result of calling this method is some kind of HTML form
     * element (input, select, textarea, etc) being returned as a String. The
     * actual element returned depends on the specifics of the Attribute it is
     * being rendered for.
     * 
     * @param data
     ...
     */
    public Type renderAttribute(RenderRequest request, RenderResponse response, Type model, String boundTo, String rowContext, String onChange, String onLoad, String renderHint)
    ...
    public String getMastersBinding(Attribute attribute) {
        ...
        return getBinding().replaceFirst("([\"'])"+attribute.getId()+"[\"']", "$1"+masterName+"$1");
    }
