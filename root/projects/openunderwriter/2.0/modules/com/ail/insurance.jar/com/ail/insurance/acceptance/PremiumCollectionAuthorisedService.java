...
public class PremiumCollectionAuthorisedService extends com.ail.core.Service<PremiumCollectionAuthorisedService.PremiumCollectionConfirmationArgument> {
    private static final long serialVersionUID = 1871676649916485145L;

    @ServiceArgument
    public interface PremiumCollectionConfirmationArgument extends Argument {
        /**
    ...
    public interface PremiumCollectionConfirmationCommand extends Command, PremiumCollectionConfirmationArgument {
    }
        ...
        for (MoneyProvision mp : policy.getPaymentDetails().getMoneyProvision()) {
            if (mp.getPaymentMethod() instanceof PayPal) {
                if (REQUESTED.equals(mp.getStatus())) {
                    ...
                    policy.getPaymentHistory().add(new PaymentRecord(mp.getAmount(), mp.getPaymentId(), mp.getPaymentMethod(), PaymentRecordType.AUTORISED));
                    requestFound = true;
                }
            } else {
