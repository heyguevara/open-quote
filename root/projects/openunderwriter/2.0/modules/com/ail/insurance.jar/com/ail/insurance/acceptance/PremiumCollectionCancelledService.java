...
/**
 * Service used to mark previously requested payments to be cancelled. Using
 * PayPal as an example, this service should be invoked if a request has been
 * sent to PayPal and rather than authorising it, the user cancelled it. In that
 * situation PayPal forward to a URL that we have specified. the handling of
 * that forward should ultimately invoke this service to ensure that the
 * policy's payment history is kept in sync with the event.
 */
@ServiceImplementation
public class PremiumCollectionCancelledService extends com.ail.core.Service<PremiumCollectionCancelledService.PremiumCollectionCancelledArgument> {
    private static final long serialVersionUID = 1871676649916485145L;

    @ServiceArgument
    public interface PremiumCollectionCancelledArgument extends Argument {
        /**
    ...
    @ServiceCommand(defaultServiceClass = PremiumCollectionCancelledService.class)
    public interface PremiumCollectionCancelledCommand extends Command, PremiumCollectionCancelledArgument {
    }
        ...
        for (MoneyProvision mp : policy.getPaymentDetails().getMoneyProvision()) {
            if (mp.getPaymentMethod() instanceof PayPal) {
                if (REQUESTED.equals(mp.getStatus())) {
                    mp.setStatus(MoneyProvisionStatus.CANCELLED);
                    policy.getPaymentHistory().add(new PaymentRecord(mp.getAmount(), mp.getPaymentId(), mp.getPaymentMethod(), PaymentRecordType.CANCELLED));
                    requestFound = true;
                }
            } else {
